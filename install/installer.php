...
if(!is_file("$mod_db")) {
	...
	$sql_messages_table = generate_sql_query("../modules/carty.mod/install/tpl-messages.php");
	$sql_prefs_table = generate_sql_query("../modules/carty.mod/install/tpl-preferences.php");
	
	$dbh = new PDO("sqlite:$mod_db");
	
	$dbh->query($sql_messages_table);
...
$sql = "SELECT * FROM prefs WHERE status = 'active' ";
...
if($prefs['version'] < $mod['version']) {

	echo '<p class="alert alert-info">an update is running ... '.$prefs['version'].' -> '.$mod['version'].'</p>';

	/* build an array from all tpl-xxx.php files in folder supplies */
	$all_tables = glob("../modules/carty.mod/install/tpl-*.php");
